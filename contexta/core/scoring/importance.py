...
class ImportanceFramework:
    ...
    _LOW_VALUE_PATTERNS = [
        re.compile(r"^\s*(hi|hello|hey|yo|sup|thanks|thank you|ok|okay|cool)\s*[!.]*\s*$", re.IGNORECASE),
        re.compile(r"^\s*(how are you|good morning|good afternoon|good evening)\s*[?.!]*\s*$", re.IGNORECASE),
        re.compile(r"^\s*(lol|haha|nice|great|awesome|sounds good)\s*[!.]*\s*$", re.IGNORECASE),
    ]
    ...
    def recency_modifier(
        ...
        *,
        ...
        if last_referenced.tzinfo is None:
            last_referenced = last_referenced.replace(tzinfo=UTC)
