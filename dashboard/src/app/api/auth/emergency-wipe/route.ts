...
export async function POST(request: Request) {
  try {
    ...
    const { email, confirmation, new_password } = payload;

    if (!email || !confirmation || !new_password) {
      return Response.json({ detail: "All fields are required." }, { status: 400 });
    }
    if (confirmation !== "WIPE") {
      return Response.json(
        { detail: "You must type 'WIPE' to confirm permanent cryptographic shredding." },
        { status: 400 }
      );
    }
    ...
    if (!res.ok) {
      let detail = "Failed to perform emergency wipe.";
      try {
