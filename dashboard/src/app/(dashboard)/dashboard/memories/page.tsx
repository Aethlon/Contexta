...
import { Database, FileText, Search, ShieldCheck, X } from "lucide-react";
import { Badge } from "@/components/ui/badge";
import { Card, CardContent, CardDescription, CardHeader, CardTitle } from "@/components/ui/card";
import { Table, TableBody, TableHead, TableHeader, TableRow } from "@/components/ui/table";
import { requireSession } from "@/lib/auth-helpers";
import { listMemoriesAction, getMemoriesAction, getGraphAction } from "@/app/actions";
    ...
    <div className="space-y-8 animate-fade-in">
      ...
      <div className="grid gap-6 lg:grid-cols-[1.3fr_0.7fr]">
        <Card>
          ...
          <CardContent>
            {query ? (
              <div className="mb-4 flex items-center justify-between rounded-lg border border-[var(--color-graphite)]/40 bg-[var(--color-ash)] px-3 py-2 text-xs font-mono">
                <span className="text-[var(--color-smoke)]">
                  Search results for: <strong className="text-[var(--color-ghost)]">{query}</strong>
                </span>
                <Link
                  href="/dashboard/memories"
                  className="inline-flex items-center gap-1 text-[var(--color-smoke)] hover:text-[var(--color-ghost)] transition-colors"
                >
